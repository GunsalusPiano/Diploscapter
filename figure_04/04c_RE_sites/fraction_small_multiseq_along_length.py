################################################################
# fraction_small_multiseq_along_length.py
# Calculates the occupancy of several small sequences 
...
# intended to count Arima RE coverage for HiC
# calculates the COVERAGE of RE sites
# user provides a small_seq_file listing the RE recognition sites, one site per line
# for Arima, the file should have the following lines:
# GATC
# GAATC
# GACTC
# GAGTC
# GATTC
#
# seq is a SeqRecord object passed from the main script
# window_size - integer of the window size (bp)
# increment - integer of the increment (bp)
# xaxis_label - string of desired x-axis label (eg. "chromosome_position")
# yaxis_label - string of desired y-axis label (eg. 
def count_fraction_of_small_seq_sites_coverage(seq, small_seq_file, window_size, increment, xaxis_label, yaxis_label, file_prefix):
    
    # Read the small sequences from the small_seq_file
    list_of_small_seqs = []
    with open(small_seq_file, "r") as small_seq_fh:
        line = small_seq_fh.readline()
        
        while line:
            list_of_small_seqs.append(line.rstrip())
            line = small_seq_fh.readline()
    
    # Grab the length of the genomic DNA to be tested
    length_of_seq = len(seq)
    
    # Initialize the values for start/end
    start = 0
    end = 0
    ...
    with open(temp_output_filename, "w") as temp_output_fh:
        ...
        while start < length_of_seq:
            # Count all RE sites in the list_of_small_seqs
            # for every RE site, the coverage is count x length_of_RE_site
            small_seq_site_cov = 0
            for small_seq_string in list_of_small_seqs:
                small_seq_site_cov = small_seq_site_cov + seq.seq.upper().count(small_seq_string, start, end)*len(small_seq_string)
...
def main(argv):
    ...
    arg_help = "\n\n{0}\n".format(argv[0]) + \
                ...
                "\n-s, or a file containing the short RE sequences to look for." + \
                "\nwindow size -w and increment -i default to 100000.\n\n"
    
    opts, args = getopt.getopt(argv[1:], "hf:s:p:x:y:w:i:", ["fasta=", "shortSequence=", "prefix=", "xaxis=", "yaxis=", "windowSize=", "increment="])
    for opt, arg in opts:
        if opt in ("-h", "--help"):
