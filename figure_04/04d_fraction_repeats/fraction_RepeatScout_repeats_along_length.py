############################################################################
# fraction_RepeatScout_repeats_along_length.py
# Pipeline to generate repeat occupancy tables. Using
# 1. RepeatScout to identify de novo repeats.
# 2. filter-stage-1.prl to eliminate likely tandem/short repeats
# 3. RepeatMasker to mark the positions of the repeat families
#    identified in steps 1 and 2.
#
# REQUIRES:
#    RepeatScout and related software in system $PATH
#    RepeatMasker and related software in system $PATH
#    rmOutToGFF3custom_20231021.bash by Darren Card and modified by me
# and
...
# pulls out the correct attribute from column 9 in a GFF file
...
def main(argv):
    ...
    arg_help = "\n\n{0}\n".format(argv[0]) + \
                ...
                "{0} [options] -f <genome fasta> -p <genome prefix> -l <l-mer l> -t <repeat occurrence threshold> -x <x-axis label> -w <window size> -i <increment>".format(argv[0]) + \
                "\n(window size and increment default to 100000, RepeatScout l-mer defaults to 14-mer, repeat occurrence threshold defaults to 5.)\n\n" + \
    ...
    opts, args = getopt.getopt(argv[1:], "hf:p:l:t:x:w:i:", ["fasta=", "prefix=", "lmer=", "threshold=", "xaxis=", "window=", "increment="])
    for opt, arg in opts:
        if opt in ("-h", "--help"):
    ...
    ##############################################################################################################################
    # STEP 1: build l-mer table (for RepeatScout)                                                                                #
    ##############################################################################################################################
    ...
    ##############################################################################################################################
    # STEP 3: Run filter-stage-1.prl from RepeatScout                                                                            #
    ##############################################################################################################################
    ...
    ##############################################################################################################################
    # STEP 4: Run RepeatMasker using the library from filter-stage-1.prl                                                         #
    ##############################################################################################################################
    ...
    ##############################################################################################################################
    # STEP 5: Count the occurrence and occupancy of each repeat family identified by RepeatScout/filter-stage-1                  #
    ##############################################################################################################################
    ...
    ##############################################################################################################################
    # STEP 6: Generate a new GFF with only repeats that occur more than THRESHOLD number of times.                               #
    ##############################################################################################################################
    ...
    ##############################################################################################################################
    # STEP 7: Generate a repeat classfication file. Usually via RepeatScout and RepeatMasker there are only two classes -        #
    # "simple_repeat" from RepeatScout and "Unspecified" from RepeatScout.                                                       #
    ##############################################################################################################################
    ...
    for seq in SeqIO.parse(arg_fasta, "fasta"):
        count_RepeatMasker_masked_content_v20231026(seq, new_gff, repeat_classification_file, arg_window, arg_increment, arg_x, file_index)
