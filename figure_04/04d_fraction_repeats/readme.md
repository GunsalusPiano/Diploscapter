...
The .bash file contains the commands to run the custom Python script ```fraction_RepeatScout_repeats_along_length.py``` to search for and annotate repetitive sequences in the *D. pachys* and the *D. coronatus* genomes. It broadly follows a repeat annotating strategy described by [Yoshida & al. (2023) *Nat Ecol Evol*](https://www.nature.com/articles/s41559-022-01980-z) for *Pristionchus* genomes by using [RepeatScout](https://github.com/Dfam-consortium/RepeatScout) and [RepeatMasker](https://www.repeatmasker.org/).
